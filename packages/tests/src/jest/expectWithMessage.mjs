// https://github.com/alexneo2003/jest-expect-message/blob/master/src/withMessage.js

const isStringOrFunction = (potentialFunctionOrString) => {
    const type = typeof potentialFunctionOrString;

    return type === 'string' || type === 'function';
};

const callIfFunction = (functionOrString) =>
    typeof functionOrString === 'function' ? functionOrString() : functionOrString;

class JestAssertionError extends Error {
    constructor(result, callsite) {
        super(callIfFunction(result.message));
...
const wrapMatcher = (matcher, customMessage) => {
    const newMatcher = (...args) => {
        try {
            ...
            if (!isStringOrFunction(customMessage)) {
                throw new JestAssertionError(matcherResult, newMatcher);
            }

            const message = () =>
                'Custom message:\n  ' +
                callIfFunction(customMessage) +
                '\n\n' +
                callIfFunction(error.matcherResult.message);
...
const expectWithMessage = (expect) => {
...
global.expect = expectWithMessage(global.expect);
